...
- **`MAX_MODEL_LENGTH = 2048`**: Reduced from the excessive 8192 limit. Evaluation data showed a maximum of 23 prompt tokens + 256 decode tokens; therefore, 2048 provided safety headroom while avoiding VRAM over-allocation.
...
When enabling the heavy optimizations above (specifically FP8 quantification metadata and APC block tables), the engine crashed with `torch.OutOfMemoryError` during PyTorch's CUDA graph capture warmup phase on the 16GB RTX 5080.

To stabilize the cluster without sacrificing concurrency, the following hardware constraints were tuned:

- **`GPU_MEMORY_UTILIZATION = 0.80`**: Dropping the default `0.90` down to `0.80`, dedicating a hard **~3.2GB buffer** of VRAM purely for the PyTorch CUDA graph allocator.
